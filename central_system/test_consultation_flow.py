#!/usr/bin/env python3
"""
Consultation Flow Test - Simplified Version

This script tests ESP32 response processing without import issues.
Run from central_system directory.

Usage:
    cd central_system
    python test_consultation_flow.py
"""
...
# Import local modules (now we're in central_system directory)
...
class SimpleMQTTTester:
    """Simple MQTT tester for consultation responses."""
    
    def __init__(self, broker_host):
        self.broker_host = broker_host
        self.client = mqtt.Client("Simple_Consultation_Tester")
    ...
    def send_button_response(self, consultation_id, response_type="ACKNOWLEDGE"):
        ...
        # Create ESP32 button response exactly as ESP32 would send it
        response_data = {
            "faculty_id": FACULTY_ID,
            "faculty_name": "Dave Jomillo",
            "response_type": response_type,
            "message_id": str(consultation_id),  # Critical: consultation ID as string
    ...
    def run_test(self):
        """Run the consultation response test."""
        logger.info("🚀 Starting Consultation Response Test...")
        ...
        # Step 3: Wait a moment for any existing subscriptions
        time.sleep(2)
        ...
        if verify_consultation_status(consultation_id, "ACCEPTED"):
        ...
        if acknowledge_success and busy_success:
            ...
            logger.info("⚠️ Partial success - some responses working")
        else:
            logger.error("❌ All tests FAILED - check central system Faculty Response Controller")
...
def main():
    """Main function."""
    logger.info("🔧 Simple Consultation Response Test")
    logger.info("=" * 50)
    ...
    # Run the test
    tester = SimpleMQTTTester(broker_host)
    ...
    try:
        ...
        if not success:
            ...
            logger.info("   1. Central system is running (python main.py)")
            logger.info("   2. Faculty Response Controller is started")
            logger.info("   3. MQTT broker configuration matches")
            logger.info("   4. Look for '🔥 FACULTY RESPONSE HANDLER TRIGGERED' in central system logs")
